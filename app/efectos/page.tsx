...
export default function EfectoPage() {
  ...
  const seleccionarCelda = async (celda: Celda) => {
    ...
    startTransition(async () => {
      ...
      if (nuevaCeldaId) {
        ...
        const { data: celdaInicial } = await supabase.from("celdas").select("efecto_id, letra_asignada").eq("id", nuevaCeldaId).single();
        if (celdaInicial) {
  ...
  const cargarTodasLasMatrices = async () => {
    ...
    if (error) {
        ...
        return setMensaje("No se pudieron cargar los eventos. Revisa los permisos de la tabla.");
    }
    setAllMatrices(data);
  ...
  const cargarCeldasDeMatriz = async (matriz: Matriz) => {
    ...
    const { data: celdasData, error: celdasError } = await supabase.from('celdas').select('id, fila, columna, estado_celda').eq('matriz_id', matriz.id).order('fila, columna');
    if (celdasError) {
        console.error("Error cargando celdas:", celdasError);
        return setMensaje("No se pudieron cargar las posiciones.");
    }
  ...
  useEffect(() => {
    ...
    if (idGuardado && infoGuardada) {
      ...
      setIsUiVisible(false); // Inicia oculto si ya tiene una celda
    } else {
      cargarTodasLasMatrices();
    }
  ...
  useEffect(() => {
    ...
    const handleWaveUpdate = ({ highlightedColumn, color }: { highlightedColumn: number | null, color: string | null }) => {
      setWaveState({ column: highlightedColumn, color: color });
    };
    socket.on('wave-update', handleWaveUpdate);
    return () => { socket.off('wave-update', handleWaveUpdate); };
  }, [socket, selectedMatriz]);
  
  useEffect(() => {
    if (!celdaId) return;
    const findAndSetMatriz = async () => {
        const { data } = await supabase.from('celdas').select('matriz_id').eq('id', celdaId).single();
        if(data) {
            const { data: matrizData } = await supabase.from('matrices').select('*').eq('id', data.matriz_id).single();
    ...
    const verificarEstado = async () => {
        const { data: miCeldaData, error: miCeldaError } = await supabase.from("celdas").select("estado_celda, efecto_id, letra_asignada").eq("id", celdaId).single();
        ...
        if (globalData && globalData.efecto_actual !== efectoGlobal) setEfectoGlobal(globalData.efecto_actual);
    };
  ...
  const [waveState, setWaveState] = useState<{ column: number | null, color: string | null }>({ column: null, color: null });
  const claseEfecto = efectoGlobal && efectoGlobal !== "inicial" ? `efecto-${efectoGlobal}` : `efecto-${efecto}`;
  ...
  if (celdaId) {
    ...
    if (finalClaseEfecto === 'efecto-mostrar-letra' && letra) {
        return ( <div className={`container-letra ${finalClaseEfecto}`}><span className="letra-display">{letra}</span></div> );
    }
    
    // --- ¡CORRECCIÓN AQUÍ! ---
    // La visibilidad ahora solo depende de `isUiVisible`.
    const showUi = isUiVisible;

    return (
      <div className={`container-confirmacion ${finalClaseEfecto}`} style={inlineStyle} onClick={handleScreenTap}>
        <div className={`info-container ${showUi ? "visible" : ""}`}>
          <h1>¡Listo!</h1><p>Tu posición está confirmada.</p><div className="luz-indicadora"></div>
        </div>
        <button onClick={liberarMiCelda} disabled={isPending} className={`boton-salir ${showUi ? "visible" : ""}`}>Salir</button>
      </div>
    );
  }
        ...
        <div className="matriz-grid-seleccion" style={{ gridTemplateColumns: `repeat(${selectedMatriz.columnas}, 1fr)` }}>
          {celdas.map((celda) => {
            ...
            return (
              <button
                key={celda.id}
                className={`celda-seleccion ${celda.estado_celda === 1 ? 'ocupada' : 'libre'} ${isHighlighted ? 'wave-active' : ''}`}
                style={isHighlighted && waveState.color ? { backgroundColor: waveState.color, borderColor: waveState.color } : {}}
    ...
    <div className="container-seleccion">
      ...
      <div className="lista-matrices">
        {allMatrices.map((matriz) => (
          <button key={matriz.id} onClick={() => cargarCeldasDeMatriz(matriz)} className="boton-matriz">{matriz.nombre}</button>
        ))}
      </div>
    </div>
  );
